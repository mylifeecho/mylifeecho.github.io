...
In order to test the bot from our local machine we would need some tunnel that will make our bot that is running localy accessible from the internet. 
Personally I use [ngrok][ngrok]. It's free for 30 requests per minute, easy to use and record interaction with your endpoint. But you can use anyone you like.
...
Now you can see http and https urls ngrok assigned to us and all requests that goes to these urls will appear here. You can also open http://localhost:4040 and see full information about requests and responses with their headers, bodies and ability to replay them again to test you server.
...
Now you can go to Telegram client and send any message you want to your bot. Bot will print it to output and you can see it in your terminal window.

# First interaction (help command)

Our bot does not do much at the moment. It does not even send any messages yet. Time to change it.
Define help message request and go to `handleUpdate` function. 
...
`handleUpdate` uses pattern matching to match on `Update` object when `handleMessage` matches on message content. You can see trick with matching on text prefix in implementation of `onCommand` function: `(Just (T.stripPrefix "/command" -> Just args)) = ...`.
...
Finally we came to the point when we will send books to the users with possibility to pay for them. 
...
Now we need to add a function to build invoice messages from books data. `sendInvoiceRequest` is a function that will create invoice request with default parameters for optional fields, but you can easily set these fields like `snd_inv_photo_url` in example below.
...
Last change is extend `handleMessage` function. For demonstration purposes we will add to commands to list all books with `/books` command and `/find <book_name_part>` to find books by title. 
To build invoice messages we would need to `map` with `buildBuyBookInvoice` function over books we want to send to the user and then we will `mapM_` with `sendInvoiceM` over invoice requests we created before.
...
Build and run it now, check that ngrok is still running. If not, start it and update your webhook url. 
Open your Telegram client and type `/books` in your bot's chat. Your bot should send you messages with *Pay* button with price for every book.
