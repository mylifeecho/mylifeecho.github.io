...
I'm going to use [haskell-telegram-api][haskell-telegram-api], Telegram Bot API bindings based on [Servant][servant] library and Servant itself to create a webhook for our bot.
Webhook based implementation makes our bot more responsive in comparison with polling based one but would require our bot to be accessible by Telegram servers. 
You can read more [here][receiving-updates].
...
You can directly answer Telegram's request to bot's webhook with an appropriate response to user action like send him a message.
...
We are going to use the later one because it makes it simpler and *telegram-api* library gives us only this option.
...
Strictly speaking this step is not required one.
We are going to create a `Bot` monad only to improve the way we read configuration and reduce the amount of boilerplate code, but for the sake of simplicity, you can pass configuration parameter manually.
